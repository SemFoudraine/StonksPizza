...
<body>
    ...
    <main>
                    ...
                    @foreach ($pizzas as $pizza)
                        <div class="pizza-card">
                            ...
                            <div class="pizza-details">
                                ...
                                <button
                                    onclick="editPizza('{{ $pizza->id }}', '{{ $pizza->name }}', '{{ $pizza->description }}', '{{ $pizza->price }}', '{{ $pizza->calories }}')"
                                    class="order-button" data-pizza="{{ $pizza->name }}">Pas nu aan</button>
                                <div class="delete-button">
                                    <form method="POST" action="{{ route('pizza.destroy', ['pizza' => $pizza->id]) }}"
                                        style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            onclick="return confirm('Weet je zeker dat je deze pizza wilt verwijderen?')">Verwijder</button>
                                    </form>
                                </div>
                            </div>
                        </div>
        ...
        <button onclick="toggleAddPizzaForm()" class="order-button" style="margin-bottom: 20px;">Pizza
            Toevoegen</button>

        <div id="addPizzaForm"
            style="display: none; background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
            ...
            <form method="POST" action="{{ route('pizza.store') }}" enctype="multipart/form-data">
                ...
                <div style="margin-bottom: 20px;">
                    <label for="name"
                        style="display: block; margin-bottom: 10px; font-weight: bold; color: #333;">Naam:</label>
                    <input type="text" id="name" name="name" required
                        style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 1rem; color: #333;">
                </div>
                <div style="margin-bottom: 20px;">
                    <label for="description"
                        style="display: block; margin-bottom: 10px; font-weight: bold; color: #333;">Beschrijving:</label>
                    <textarea id="description" name="description" rows="3" required
                        style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 1rem; color: #333;"></textarea>
                </div>
                <div>
                ...
                <div style="margin-bottom: 20px;">
                    <label for="price"
                        style="display: block; margin-bottom: 10px; font-weight: bold; color: #333;">Prijs:</label>
                    <input type="number" id="price" name="price" min="0" step="0.01" required
                        style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 1rem; color: #333;">
                </div>
                <div style="margin-bottom: 20px;">
                    <label for="calories"
                        style="display: block; margin-bottom: 10px; font-weight: bold; color: #333;">Calorieën:</label>
                    <input type="number" id="calories" name="calories" min="0" required
                        style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 1rem; color: #333;">
                </div>
                <div style="margin-bottom: 20px;">
                    <label for="image"
                        style="display: block; margin-bottom: 10px; font-weight: bold; color: #333;">Afbeelding:</label>
                    <input type="file" id="image" name="image" accept="image/*" required
                        style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 1rem; color: #333;">
                </div>
                <button type="submit"
                    style="padding: 10px 20px; font-size: 1rem; color: #fff; background-color: #007bff; border: none; border-radius: 5px; cursor: pointer; transition: background-color 0.3s;">Toevoegen</button>
            </form>
        </div>
    ...
    <script>
        function toggleAddPizzaForm() {
            ...
            addPizzaForm.style.display = addPizzaForm.style.display === 'none' || addPizzaForm.style.display === '' ?
                'block' : 'none';
        }
